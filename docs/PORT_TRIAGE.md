...
## §13 — I measured `main` while standing on a feature branch

Found 2026-07-27, at the end of the day, in my own hands, while doing the *one* task I had queued
specifically because unpushed work is how this project loses things.

I ran `git rev-parse main`, `git rev-list --count main..origin/main`, and `git push origin main`.
Every number came back clean and every one of them was about a branch this checkout **is not on**.
`HEAD` was `feat/phase4-resurface-parity`. The local `main` ref was a stale pointer nobody had moved
in days, so:

    ahead: 0            true of main, irrelevant to my work
    push exit: 1        rejected — pushing a stale ref at an advanced remote
    unpushed now: 0     true of main, and the most dangerous line of the three

That third line is the one that would have ended the day. It reads exactly like "your work is
safe." My actual work — 214 lines across `port-audit.mjs` and this file — was on a branch that
`git branch -r --contains` placed on **no remote at all**. One disk. The same shape as the seven
commits that sat unreachable behind PR #71 this morning.

Two follow-on errors, both worth recording because both are the failure *diagnosing* the failure.

**I concluded from the counts that another session had reverted me.** One of the four commits I was
behind is titled *"the rename that must not be recorded"*, which fit the story so well I nearly
wrote it up as a cross-session disagreement. There was no disagreement. Nobody reverted anything.
A plausible narrative arrived before the measurement did.

**Two of my three verification greps were wrong, in opposite directions.** `from:\s*'` counted 3 of
13 renames, because 10 are generated by a `.map()` over backtick templates. `^## §?1[012] ` found 0
of 3 sections that were sitting at lines 685, 727 and 799. I only escaped because the two wrong
greps disagreed with each other loudly enough to force a `git diff --stat`, which cannot be fooled
by a pattern. **Verify with a tool that reports structure, not one that reports matches.**
...
The class, extended:
    ...
    an error list is a floor, not a total
    the compiler is the oracle — except where a cast erases the dependency
    A BRANCH'S NAME IS NOT THE BRANCH YOU ARE ON

**Bearing on the `measurementRevision` proposal (node-platform decision #11).** That proposal already
says a dirty working tree must never satisfy the check. This is the sibling defect and it needs
naming in the same schema: **a checkout must be pinned to an immutable SHA, and the measurement must
assert `HEAD` resolves to it.** Both of today's measurement bugs — the dashboard scoring a
130-commit-stale checkout, and the port audit scoring a dirty tree — plus this one are the same
root: *the thing measured was never bound to the thing named.*

**Cheap standing check**, since the expensive lesson is already paid for:

    git branch -r --contains HEAD    # empty output = this work exists on one disk

Run it before believing any "unpushed: 0". It answers the question the count only appears to.
