...
for (const entry of await safeDir(LEDGER)) {
  ...
  add({
    id,
    label: entry.name.replace(/^thread-|\.md$/g, ''),
    group: 'thread',
    status: ageDays > 2 ? 'stale' : 'live',
    size: 8 + Math.min(turns, 30) / 4,
...
for (const proj of await safeDir(PROJECTS)) {
  ...
  for (const f of files) {
    try {
      newest = Math.max(newest, (await stat(path.join(dir, f.name))).mtimeMs);
    } catch {
      /* skip */
    }
  }
  ...
  add({
    id,
    label: proj.name.replace(/^[A-Z]--VSCode-Projects-/, '').slice(0, 28),
    group: 'session',
    status: ageDays < 1 ? 'live' : ageDays < 7 ? 'stale' : 'cold',
  ...
  const mem = (await safeDir(path.join(dir, 'memory'))).filter(
    (f) => f.isFile() && f.name !== 'MEMORY.md',
  );
  if (mem.length > 0) {
    const mid = `m:${proj.name}`;
    add({
      id: mid,
      label: `${mem.length} memories`,
      group: 'memory',
      status: 'live',
      size: 5 + mem.length / 4,
      note: mem
...
for (const row of status.split('\n')) {
  const m = row.match(
    /^\|\s*([A-Za-z][\w-]*)\s*\|\s*(\w+)\s*\|\s*([\w-]+)\s*\|.*?\|\s*(\w[\w-]*)\s*\|\s*(\d+)\/8\s*\|?\s*$/,
  );
  if (!m) continue;
  ...
  add({
    ...
    size: 7 + score,
    note: `${role} · ${life} · P0 ${score}/8 · proof ${proof.toLowerCase()}`,
  });
  link('repos', id);
...
/* ---------- evolution ledger ---------- */
const evDrafts = (await safeDir(path.join(PLATFORM, 'evolution/drafts'))).filter((f) =>
  f.isFile(),
).length;
let evEvents = 0;
for (const track of await safeDir(path.join(PLATFORM, 'evolution/events'))) {
  if (track.isDirectory()) {
    evEvents += (await safeDir(path.join(PLATFORM, 'evolution/events', track.name))).filter((f) =>
      f.isFile(),
    ).length;
  }
}
add({
  ...
  group: 'source',
  status: existsSync(path.join(PLATFORM, 'evolution/ledger.json')) ? 'live' : 'missing',
  size: 20,
...
add({
  ...
  group: 'join',
  status: graphOn ? 'blocked' : 'missing',
  size: 18,
  note: graphOn
...
const graph = {
  ...
  counts: {
    ...
    evolutionEvents: evEvents,
    evolutionDrafts: evDrafts,
