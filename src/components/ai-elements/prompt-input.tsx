    ...
    (f: File) => {
      ...
      return patterns.some((pattern) => {
        if (pattern.startsWith('.')) {
          return f.name.toLocaleLowerCase().endsWith(pattern.toLocaleLowerCase());
        }
        if (pattern.endsWith('/*')) {
          // e.g: image/* -> image/
  ...
  const inner = (
    <>
      <input
        {...fileInputProps}
        accept={accept}
        aria-label={fileInputProps?.['aria-label'] ?? 'Upload files'}
        className={cn('hidden', fileInputProps?.className)}
        multiple={multiple}
