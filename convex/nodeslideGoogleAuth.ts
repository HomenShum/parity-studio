...
export const complete = internalAction({
  ...
  handler: async (ctx, args): Promise<{ redirectTo: string }> => {
    ...
    if (!session || session.consumedAt !== undefined || session.expiresAt <= Date.now()) {
      const fallbackOrigin = config.allowedOrigins[0] ?? 'https://parity-studio.vercel.app';
      return {
        redirectTo: withGoogleOAuthResult(fallbackOrigin, 'expired'),
      };
    }
...
function requireGoogleOAuthConfig() {
  ...
  const encryptionKey = process.env['NODESLIDE_OAUTH_TOKEN_ENCRYPTION_KEY']?.trim();
  const redirectUri = process.env['NODESLIDE_GOOGLE_REDIRECT_URI']?.trim();
  const allowedOrigins = allowedNodeSlideOrigins(process.env['NODESLIDE_APP_ORIGINS']);
  if (!clientId || !clientSecret || !encryptionKey || !redirectUri || !allowedOrigins.length) {
    throw new Error('Google Slides connection is not configured for this deployment.');
  }
  return { clientId, clientSecret, encryptionKey, redirectUri, allowedOrigins };
}
